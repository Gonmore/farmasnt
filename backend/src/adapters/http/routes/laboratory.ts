...
export async function registerLaboratoryRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/api/v1/laboratory/supplies', { preHandler: writeGuard }, async (request, reply) => {
    ...
    // If initialStock is specified and no locationId provided, pick any active location.
    // (The Location model does not have an `isDefault` flag.)
    ...
    if (parsed.data.initialStock && parsed.data.initialStock > 0 && !locationId) {
      const fallback = await db.location.findFirst({
        ...
        orderBy: [{ warehouseId: 'asc' }, { code: 'asc' }, { id: 'asc' }],
        select: { id: true },
      })
      if (fallback) locationId = fallback.id
    }

    if (parsed.data.initialStock && parsed.data.initialStock > 0 && !locationId) {
      return reply.status(400).send({
        message: 'No location available for initial stock. Provide locationId or create a location first.',
      })
    }
    ...
    const created = await db.$transaction(async (tx) => {
      ...
      if (parsed.data.initialStock && parsed.data.initialStock > 0 && locationId) {
        await createSupplyStockMovementTx(tx, {
          tenantId,
          userId: actorUserId,
          type: 'IN',
