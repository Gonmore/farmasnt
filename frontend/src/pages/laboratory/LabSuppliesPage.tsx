...
export function LabSuppliesPage() {
  ...
  const suppliesQuery = useQuery({
    queryKey: ['laboratory', 'supplies', { take, cursor }],
  ...
  const updateMutation = useMutation({
    mutationFn: () => {
      if (!editing) throw new Error('Seleccioná un insumo')
    ...
    <MainLayout navGroups={navGroups}>
      <PageContainer title="🧪 Laboratorio — Materia prima">
        ...
        {suppliesQuery.isLoading ? (
          ...
          <ErrorState message={(suppliesQuery.error as any)?.message ?? 'Error al cargar insumos'} />
        ) : (
          <>
        ...
        <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Nueva materia prima" maxWidth="lg">
